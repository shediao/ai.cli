...
void StreamOperator::parse(std::string_view chunk) {
  ...
  response_.response_body_.insert(response_.response_body_.end(), begin(chunk),
                                  end(chunk));
  while (true) {
    auto line = getLine();
    if (!line.has_value()) {
    ...
    try {
      ...
      if (data_json.contains("choices")) {
        if (auto& choices_json = data_json["choices"];
          ...
          if (delta_json.contains("role") && delta_json["role"].is_string()) {
            choice.message_.role = delta_json["role"].get<std::string>();
          }
          ...
          if (delta_json.contains("reasoning_content") &&
            ...
            if (choice.message_.reasoning_content.has_value()) {
              ...
              choice.message_.reasoning_content = reasoning_content_str;
            }
          }
              ...
              if (first_tool_call.contains("type") &&
                ...
                if (!message_.contains("role")) {
                  message_["role"] = "assistant";
                         ...
                         first_tool_call.contains("function") &&
                         first_tool_call["function"].is_object() &&
                         first_tool_call["function"].contains("arguments")) {
                auto arguments =
                    choice.message_.tool_calls
                        .value()[0]["function"]["arguments"]
                        .get<std::string>() +
                    first_tool_call["function"]["arguments"].get<std::string>();
                choice.message_.tool_calls.value()[0]["function"]["arguments"] =
                    arguments;
                message_["tool_calls"][0]["function"]["arguments"] = arguments;
              }
            }
          }
          if (first_choice_json.contains("finish_reason") &&
            ...
            choice.finish_reason_ =
                first_choice_json["finish_reason"].get<std::string>();
          ...
          if (first_choice_json.contains("usage")) {
            ...
            response_.usage_.prompt_tokens_ = get_int_value("prompt_tokens");
            response_.usage_.total_tokens_ = get_int_value("total_tokens");
          }
        }
      }
